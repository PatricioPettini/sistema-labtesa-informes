...
function _fmtTension(mpa) {
  // Excel: ROUND(..., 0) en los 3 tramos (< 500, 500-1000, > 1000).
  if (isNaN(mpa)) return '';
  return String(Math.round(mpa));
}
...
function TraccionForm(props) {
        ...
        _r('thead', null,
          _r('tr', { style: { background: '#e6e6e6' } },
            ...
            idxFisicas.map(function (probIdx, iFis) {
              ...
              // Título de la columna: N° de OT al que pertenece la probeta.
              // Si tiene override (multi-OT) → esa; si no → la OT del ensayo.
          ...
          TRACCION_PARAMS.map(function (p) {
            ...
            return _r('tr', { key: p.k },
              ...
              muestras.map(function (m, i) {
                var val = (m && m[p.k]) || '';
                var esZona = !!(m && m._zona_extra);
                // Solo se marca con verde (viene del cálculo) si el campo está
                // en la lista de calculados Y tiene valor. Sino, editable normal.
                var vieneDeCalc = esCalc && val !== '';
                var bg = vieneDeCalc ? '#e6f9ef' : (esZona ? '#fffbef' : 'transparent');
                return _r('td', { key: i, style: { border: '1px solid #333', padding: 0, background: bg } },
                  _r('input', {
                    style: Object.assign({}, S.input, S.num, {
                      ...
                      color: vieneDeCalc ? '#0a7a55' : undefined,
                      fontWeight: vieneDeCalc ? 700 : undefined,
                    }),
                    value: val,
                    title: esCalc ? 'Calculado desde 1.1 si hay datos; editable si querés override manual' : '',
  ...
  function scBlock(idxM) {
    ...
    return _r('div', { key: idxM, style: cardStyleZona },
      ...
      _r('div', { style: scStyles.cardBody },
        ...
        esZona
          ...
          : null,
        // ── Mediciones dimensionales (3 valores + promedio) ──
        tipo === 'rect' ? scDimRow(idxM, 'A', ['a1', 'a2', 'a3'], 'mm') : null,
        tipo === 'rect' ? scDimRow(idxM, 'E', ['e1', 'e2', 'e3'], 'mm') : null,
        tipo === 'cil'  ? scDimRow(idxM, 'D', ['d1', 'd2', 'd3'], 'mm') : null,

        // ── Secciones S1/S2/S3 por medición individual (readonly) ──
        // Sj = Aj × Ej (rect) o π × Dj² / 4 (cil). Sale del Excel FM-044.
        (function () {
          var secciones = calcSeccionesFM044(seccionCalc[idxM] || {});
          return _r('div', { style: scStyles.sRow },
            ['S1', 'S2', 'S3'].map(function (lbl, j) {
              var v = secciones[j];
              var vacio = !v;
              return _r('div', { key: lbl,
                style: Object.assign({}, scStyles.sBox, vacio ? {} : { background: '#e6f9ef', borderColor: '#a8d9c1' }) },
                _r('span', { style: scStyles.sLabel }, lbl),
                _r('span', { style: {
                  flex: 1, textAlign: 'center', fontSize: 11,
                  fontWeight: 700, color: vacio ? '#c0c0c0' : '#0a7a55',
                } }, vacio ? '—' : v),
                _r('span', { style: scStyles.sUnit }, 'mm²'));
            }));
        })(),

        // ── Cargas y longitudes ──
        _r('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginTop: 4 } },
          inputRow('Carga rot.',  'fr', 'DaN'),
          inputRow('Carga flu.',  'ff', 'DaN'),
          inputRow('Long. inic.', 'l0', 'mm'),
          inputRow('Long. final', 'lf', 'mm'),
          tipo === 'cil' ? inputRow('Diám. final', 'df', 'mm') : null
        ),

        // ── Extremos para verificación PASA/NO PASA ──
        _r('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 4, marginTop: 4, alignItems: 'center' } },
          inputRow(extLabel + ' 1', 'ext1', 'mm'),
          inputRow(extLabel + ' 2', 'ext2', 'mm'),
          verif
            ? _r('span', {
                style: {
                  padding: '3px 10px', borderRadius: 12, fontSize: 10, fontWeight: 700,
            ...
            : _r('span', { style: { fontSize: 10, color: '#8a8a8a', fontStyle: 'italic' } }, 'verif —')
        ),

        // ── Resultados calculados (readonly) ──
        _r('div', { style: { marginTop: 6, paddingTop: 6, borderTop: '1px dashed #d0d7de' } },
          _r('div', { style: { fontSize: 9, color: '#8a8a8a', textTransform: 'uppercase', letterSpacing: '.03em', marginBottom: 4, fontWeight: 700 } }, 'Calculado automáticamente'),
          _r('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 } },
            tipo === 'rect' ? calcRow('Ancho prom.', 'ancho_promedio', 'mm') : null,
            tipo === 'rect' ? calcRow('Espesor prom.', 'espesor_promedio', 'mm') : null,
            tipo === 'cil'  ? calcRow('Diám. prom.', 'diametro_promedio', 'mm') : null,
            calcRow('S₀',             'seccion_inicial',       'mm²'),
            calcRow('Tens. rotura',   'resistencia_traccion',  'MPa'),
            calcRow('Tens. fluencia', 'tension_fluencia',      'MPa'),
            calcRow('Alargamiento',   'alargamiento',          '%'),
            tipo === 'cil' ? calcRow('Sf',          'seccion_final', 'mm²') : null,
            tipo === 'cil' ? calcRow('Estricción',  'estriccion',    '%')   : null
          ))
      )
    );
  }
